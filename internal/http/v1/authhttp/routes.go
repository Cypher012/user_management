...
func RegisterAuth(r chi.Router, pool *pgxpool.Pool, jwt *auth.JWTAuth, email *email.Service, token *token.TokenService) {
	repo := auth.NewAuthRepository(pool)
	service := auth.NewAuthService(repo, token, email)
	...
	r.Route("/v1/auth", func(r chi.Router) {
		...
		r.Post("/verify-email/resend", func(w http.ResponseWriter, r *http.Request) {})
